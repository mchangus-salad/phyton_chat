...
class WeaviateRetriever:
    ...
    def _reconnect(self) -> None:
        """Re-establish the Weaviate client connection after an idle or dropped channel."""
        import weaviate
        from weaviate.auth import AuthApiKey
    ...
    def _query_vector(self, vector: list[float]):
        """Execute a near_vector query, retrying once after reconnection on failure."""
        try:
            ...
            logger.warning("Weaviate query failed (%s), reconnecting and retrying.", exc)
            self._reconnect()
            return self.collection.query.near_vector(
                near_vector=vector,
                limit=settings.max_context_docs,
    ...
    def invoke(self, query: str):
        vector = self.embeddings.embed_query(query)
        try:
            response = self._query_vector(vector)
        except Exception as exc:
            logger.error("Weaviate retrieval failed after retry: %s", exc)
            return []
        docs = []
