...
export function useOncologyQuery() {
  const { t } = useI18n();
  ...
  async function query(question, authToken, tenantId) {
    ...
    try {
      ...
      let streamedAnswer = '';
      const response = await apiPostNdjson('/agent/oncology/query/stream/', { question }, {
        headers,
        onEvent: (event) => {
        ...
        if (streamErr?.status === 404 || streamErr?.status === 405) {
          return apiPost('/agent/oncology/query/', { question }, { headers });
        }
        throw streamErr;
      });
      ...
      const normalized = response?.event === 'done'
        ? {
            ...
            domain: response.domain || 'oncology',
