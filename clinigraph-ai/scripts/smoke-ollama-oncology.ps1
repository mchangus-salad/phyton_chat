param(
    ...
    [string]$Subdomain = "lung-cancer",
    [string]$EvidenceType = "guideline",
    [string]$Question = "Summarize key biomarkers for lung cancer research from available context.",
    [string]$ApiKey = ""
)
...
$sampleCorpus = Join-Path $repoRoot "data\oncology_sample.json"
...
Write-Step "3/5 Import oncology corpus"
& $venvPython manage.py import_oncology_corpus $sampleCorpus --subdomain $Subdomain
if ($LASTEXITCODE -ne 0) {
    Write-Fail "Corpus import failed"
}
Write-Ok "Corpus imported into subdomain $Subdomain"
...
Write-Step "4/5 Run oncology evidence search"
$evidenceBody = @{
...
$evidenceResponse = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/v1/agent/oncology/evidence/" -Headers $headers -Body $evidenceBody
if (-not $evidenceResponse) {
    Write-Fail "No response from oncology evidence endpoint"
}
Write-Ok "Evidence endpoint returned $($evidenceResponse.evidence.Count) document(s)"

Write-Step "5/5 Run oncology query"
$queryBody = @{
    question = $Question
    subdomain = $Subdomain
    user_id = "smoke-ollama-oncology"
} | ConvertTo-Json

$queryResponse = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/v1/agent/oncology/query/" -Headers $headers -Body $queryBody
if (-not $queryResponse -or [string]::IsNullOrWhiteSpace($queryResponse.answer)) {
    Write-Fail "No answer returned by oncology query endpoint"
}
Write-Ok "Oncology query endpoint returned an answer"
